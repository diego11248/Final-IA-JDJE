...
def main():
    ...
    train_and_evaluate(lstm_net, "Modelo LSTM (PyTorch)", train_loader, test_loader, y_test, epochs=EPOCHS)
    
    # 3. Inicialización y entrenamiento del Transformer
    transformer_net = TransformerModel(input_dim=features, timesteps=timesteps)
    train_and_evaluate(transformer_net, "Modelo Transformer (PyTorch)", train_loader, test_loader, y_test, epochs=EPOCHS)
